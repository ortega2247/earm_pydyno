...
from models.earm_kochen_original_ic import model
...
if __name__ == '__main__':
    ...
    sampled_params, log_ps = run_dream(parameters=sampled_parameter_names, likelihood=likelihood, nCR=6,
                                       ...
                                       model_name='earm_kochen_original_dreamzs_5chain2', verbose=False)

    # Save sampling output (sampled parameter values and their corresponding logps).
    for chain in range(len(sampled_params)):
        np.save('earm_kochen_original_dreamzs_5chain_sampled_params_chain_' + str(chain)+'_'+str(total_iterations), sampled_params[chain])
        np.save('earm_kochen_original_dreamzs_5chain_logps_chain_' + str(chain)+'_'+str(total_iterations), log_ps[chain])

    #Check convergence and continue sampling if not converged

    GR = Gelman_Rubin(sampled_params)
    print('At iteration: ',total_iterations,' GR = ',GR)
    np.savetxt('earm_kochen_original_dreamzs_5chain_GelmanRubin_iteration_'+str(total_iterations)+'.txt', GR)
    ...
    if np.any(GR>1.2):
        while not converged:
            ...
            sampled_params, log_ps = run_dream(parameters=sampled_parameter_names, likelihood=likelihood,
                                               ...
                                               adapt_gamma=True, history_thin=1, model_name='earm_kochen_original_dreamzs_5chain2',
    ...
    try:
        ...
        for dim in range(ndims):
            ...
            fig.savefig('PyDREAM_earm_kochen_original_dimension_'+str(dim))
    ...
    run_kwargs = {'parameters':sampled_parameter_names, 'likelihood':likelihood, 'niterations':niterations, 'nchains':nchains, \
                  'multitry':False, 'gamma_levels':4, 'adapt_gamma':True, 'history_thin':1, 'model_name':'earm_kochen_original_dreamzs_5chain2', 'verbose':False}
